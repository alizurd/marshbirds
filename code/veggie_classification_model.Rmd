...
1. NDWI + NDVI + PCA + NAIP (no brightness)
2. NDWI + NDVI + PCA
3. NDVI + PCA
4. NAIP only
5. PCA only
...
knitr::opts_knit$set(root.dir = "/Users/alyssabueno/Desktop/marshbirdsoutput/round_17") # increase by 1 everytime you run a new model

out_dir <- "~/Desktop/marshbirdsoutput/round_17"
...
raster_files <- c(
  "/Users/alyssabueno/Desktop/marshbirds_data/training_clips/training_clip_bx.tif",
  "/Users/alyssabueno/Desktop/marshbirds_data/training_clips/training_clip_li.tif",
  "/Users/alyssabueno/Desktop/marshbirds_data/training_clips/training_clip_gb.tif"
)
...
for (i in 1:length(rasters)) { # starting a loop over all rasters (rasters has 3 rasters, so i = 1,2,3)
  ...
  ndwi <- (naip[[2]] - naip[[4]]) / (naip[[2]] + naip[[4]]) # McFeeters: (Green - NIR) / (Green + NIR)
  names(ndwi) <- "ndwi"
...
# set mud and open water as the same class = ow
extracted_clean$class <- ifelse(extracted_clean$class %in% c("md", "sd"), "sd", extracted_clean$class)
extracted_clean$class <- as.factor(extracted_clean$class)
...
for (i in 1:length(test_rasters)) {
  ...
  ndwi <- (naip[[2]] - naip[[4]]) / (naip[[2]] + naip[[4]])
  names(ndwi) <- "ndwi"
...
# set mud and sand as the same class = sd (matching training)
extracted_test$class <- ifelse(extracted_test$class %in% c("md", "sd"), "sd", extracted_test$class)
extracted_test$class <- as.factor(extracted_test$class)
...
# ITERATION 1: Full Model (NAIP + NDVI + NDWI + PCA)

```{r}
cat("\n========== ITERATION 1: Full Model ==========\n")

# Use all columns from training_data_full except brightness
rf_model_1 <- randomForest(class ~ . - brightness,
...
train_2 <- training_data_full %>% 
  dplyr::select(ndwi, ndvi, starts_with("PCA"), class)
...
train_3 <- training_data_full %>%
  dplyr::select(ndvi, starts_with("PCA"), class)
...
# Save model
saveRDS(rf_model_3, "~/Desktop/marshbirds_data/rf_model_3_ndvi_pca.rds")
```
...
train_4 <- training_data_full %>% 
  dplyr::select(starts_with("naip"), class)
...
train_5 <- training_data_full %>% 
  dplyr::select(starts_with("PCA"), class)
...
# PART 3: Classify New Rasters (Choose Model)
...
if (CHOSEN_MODEL == 1) {
  rf_model <- rf_model_1
  feature_cols <- names(training_data_full)[!names(training_data_full) %in% c("class", "brightness")]
} else if (CHOSEN_MODEL == 2) {
  rf_model <- rf_model_2
  feature_cols <- c("ndwi", "ndvi", paste0("PCA", 1:5))
} else if (CHOSEN_MODEL == 3) {
  rf_model <- rf_model_3
  feature_cols <- c("ndvi", paste0("PCA", 1:5))
...
# -------------------------------------------------------------------
# Process each raster — feature calculations mirror the original code
# -------------------------------------------------------------------

for (f in files) {
  message("Processing: ", basename(f))
  
  # Load raster (called 'naip' to match original code exactly)
  naip <- rast(f)
  names(naip) <- paste0("naip", 1:4)
  
  # --- Calculate NDVI (same formula as original) ---
  ndvi <- (naip[[4]] - naip[[1]]) / (naip[[4]] + naip[[1]])
  names(ndvi) <- "ndvi"
  
  # --- Calculate NDWI (same formula as original) ---
  ndwi <- (naip[[2]] - naip[[4]]) / (naip[[2]] + naip[[4]])
  names(ndwi) <- "ndwi"
  
  # --- Calculate Brightness (same formula as original) ---
  brightness <- (naip[[1]] + naip[[2]] + naip[[3]] + naip[[4]]) / 4
  names(brightness) <- "brightness"
  
  # --- Apply PCA using the SAME pca_model saved during training ---
  # Input must match what pca_model was trained on: c(naip, ndvi)
  pca_input <- c(naip, ndvi)
  pca <- predict(pca_input, pca_model, index = 1:5)
  names(pca) <- paste0("PCA", 1:5)
  
  # --- Stack all layers (same order as training full_stack) ---
  full_stack <- c(naip, ndvi, ndwi, brightness, pca)
  
  # --- Select only the features this model needs ---
  prediction_stack <- full_stack[[feature_cols]]
  
  # --- Rename to match training names exactly ---
  names(prediction_stack) <- feature_cols
  ...
  # Clean up memory between tiles
  rm(naip, ndvi, ndwi, brightness, pca_input, pca, full_stack, prediction_stack, classified)
  gc()
}
...
example_file <- 
  # list.files(
                          "~/Desktop/marshbirdsoutput/round_17/e_montauk_raster_200_model2_classified.tif"
...
# Load shapefile and set CRS to match rasters
plots <- st_read("~/Desktop/marshbirds_data/buffers/buffer_all_200.shp")
plots <- st_transform(plots, crs(rast(classified_files[1])))  # reproject to match rasters
cat("Shapefile loaded. Number of plots:", nrow(plots), "\n")

# Build lookup table: terra's sequential ID -> point_id
plot_lookup <- plots %>%
  st_drop_geometry() %>%
  mutate(ID = row_number()) %>%
  dplyr::select(ID, point_id)

# Convert to terra format
plots_terra <- vect(plots)

# Class mapping: integer codes -> class names
class_mapping <- data.frame(
  class_code = c(1, 2, 3, 4, 5, 6, 7, 8),
  class_name = c("hm", "lm", "ow", "ph", "rd", "up", "md", "sd")
)

# Extract pixel values from each classified raster tile
...
for (i in seq_along(classified_files)) {
  ...
  tryCatch({
    ...
    message("Error on tile ", basename(classified_files[i]), ": ", e$message)
  })
}

# Combine all tiles
all_values <- bind_rows(all_extractions)
cat("Total pixels extracted:", nrow(all_values), "\n")

# Join point_id using the lookup table (safe, explicit join)
all_values <- all_values %>%
  left_join(plot_lookup, by = "ID")

# Map integer class codes to class names
all_values <- all_values %>%
  mutate(class = as.numeric(class)) %>%
  left_join(class_mapping, by = c("class" = "class_code")) %>%
  dplyr::select(-class) %>%
  rename(class = class_name) %>%
  filter(!is.na(class), !is.na(point_id))

# Calculate percent cover per plot
percents <- all_values %>%
  group_by(point_id, class) %>%
  summarise(pixel_count = n(), .groups = "drop") %>%
  group_by(point_id) %>%
  mutate(percent = pixel_count / sum(pixel_count)) %>%
  ungroup()

# Pivot to wide format: one row per plot, one column per class
percent_cover <- percents %>%
  dplyr::select(point_id, class, percent) %>%
  pivot_wider(
    names_from = class,
    values_from = percent,
    values_fill = 0
...
cat("Final table dimensions:", nrow(percent_cover), "plots x", ncol(percent_cover), "columns\n")
View(percent_cover)

write.csv(percent_cover,
          "~/Desktop/percent_cover_by_plot_jan.csv",
          row.names = FALSE)
```
...
# Summary table
results <- data.frame(
  Model = c("Full Model", "NDWI+NDVI+PCA", "NDVI+PCA", "NAIP Only", "PCA Only"),
  Features = c(
    "NAIP1-4, NDVI, NDWI, PCA1-5",
    "NDWI, NDVI, PCA1-5",
    "NDVI, PCA1-5",
    "NAIP1-4",
    "PCA1-5"
