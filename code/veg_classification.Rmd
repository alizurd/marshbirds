...
# turn class into a factor and add to extracted data
polygon$class <- as.factor(polygon$class)
poly_df <- as.data.frame(polygon)
poly_df$ID <- 1:nrow(poly_df)

# Join the class labels
extracted_i <- extracted_i %>%
...
training_data <- extracted_clean %>%
  group_by(class) %>%
  sample_n(min(3000, n())) %>%  # Use min() to handle small classes
  ungroup()
