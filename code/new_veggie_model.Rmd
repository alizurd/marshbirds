...
# class_percentages <- function(raster_path) {
#   r <- rast(raster_path)
#   freq_table <- freq(r, useNA = "no")
#   total_pixels <- sum(freq_table[, 2])
#   percentages <- (freq_table[, 2] / total_pixels) * 100
#   class_labels <- c("hm", "lm", "md", "ow", "ph", "rd", "sd", "up")
  
#   result <- data.frame(
#     Class = class_labels[freq_table[, 1]],
...
# load shp first
plots <- st_read("~/Desktop/marshbirdsoutput/round_7/200m_buffer.shp") # need to upload all of the shapefiles in a for loop(?)
